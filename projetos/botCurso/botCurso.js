...
bot.start(async ctx => {
    ...
    await ctx.replyWithMarkdown(`_Posso te ajudarem algo?_`, tecladoOpcoes)
})
...
bot.hears('O que verei no curso?', async ctx => {
    ...
    await ctx.reply('2. Um bot que vai permitir cadastrar os seus eventos')
...
bot.hears('Como comprar o curso?', ctx => {
    ctx.replyWithMarkdown('Que bom...', tecladoOpcoes)
})
...
bot.action('s', async ctx => {
    await ctx.reply(`Que legal, tente me enviar a sua localização, ou escrever qualaquer coisa`, localizacao)
})

bot.hears(/qualaquer coisa/i, ctx => {
...
bot.on('text', async ctx => {
    ...
    await ctx.reply(`A sua mensagem ai contrário é ${msg}`)
    await ctx.reply('Isso mostra que consigo ler o que você escreve e processar sua mensagem', tecladoOpcoes)
})
...
bot.on('location', async ctx => {
    try {
        ...
        // console.log(lat, lon)
        const res = await axios.get(`${url}?lat=${lat}&lon=${lon}&APPID=${env.weatherApiKey}&units=metric`)
        await ctx.reply(`Hmmm... você está em ${res.data.name}`)
        await ctx.reply(`A temperatura por aí está em ${res.data.main.temp}°C`, tecladoOpcoes)
    } catch (e) {
        console.error(e)
        ctx.reply('Estou tendo problemas para obter a temperatura para a sua localização. Você está no planeta Terra? :P', tecladoOpcoes)
    }
})
